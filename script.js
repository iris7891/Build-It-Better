document.addEventListener("DOMContentLoaded", function () {
  const materials = {
    steel: {
      ...
      engineerNote: "Steel has high strength and stiffness, but it has high density, so it is heavier than aluminum, wood, or plastic."
    },
    aluminum: {
      name: "Aluminum",
      strength: 7,
      ...
      engineerNote: "Aluminum has a good strength-to-weight ratio, which makes it useful for bikes, airplanes, and lightweight structures."
    },
    wood: {
      name: "Wood",
      strength: 5,
      ...
      engineerNote: "Wood has moderate strength and low density. It is also renewable, which can make it useful for sustainable design."
    },
    plastic: {
      name: "Plastic",
      strength: 3,
      ...
      engineerNote: "Many plastics are low-density and low-cost, but they usually have lower stiffness and strength than metals."
    },
    rubber: {
      name: "Rubber",
      strength: 2,
      ...
      engineerNote: "Rubber is useful when flexibility and impact absorption matter more than stiffness or strength."
    },
    bamboo: {
      name: "Bamboo",
      strength: 6,
      ...
      engineerNote: "Bamboo is lightweight and renewable, so it is useful for teaching sustainable material choices."
    },
    concrete: {
      name: "Concrete",
      strength: 8,
      ...
      engineerNote: "Concrete is strong when squeezed, which is called compression, but it is heavy and not flexible."
    },
    cardboard: {
      name: "Cardboard",
      strength: 2,
      ...
      engineerNote: "Cardboard is useful for prototypes because it is inexpensive, lightweight, and easy to cut."
    },
    carbonfiber: {
      name: "Carbon Fiber",
      strength: 9,
      ...
      engineerNote: "Carbon fiber has excellent strength-to-weight performance, but it is costly and harder to recycle."
    }
  };
  ...
  const missions = {
    bridge: {
      ...
      checklist: { strength: 6, stiffness: 6, cost: 4 }
    },
    phone: {
      ...
      checklist: { flexibility: 6, lightness: 6, cost: 4 }
    },
    toycar: {
      ...
      checklist: { lightness: 6, cost: 6, strength: 4 }
    },
    bike: {
      ...
      checklist: { strength: 6, lightness: 6, stiffness: 5 }
    },
    playground: {
      ...
      checklist: { strength: 6, stiffness: 5, cost: 4 }
    }
  };
  ...
  function loadDropdown(selectElement, defaultValue) {
  ...
  function createChecklist(material, mission) {
    ...
    for (const property in mission.checklist) {
      const needed = mission.checklist[property];
      const actual = material[property];

      if (actual >= needed) {
        html += `<div class="check-item pass">✓ ${capitalize(property)} passed: ${actual}/10</div>`;
      } else {
        html += `<div class="check-item fail">✗ ${capitalize(property)} needs improvement: ${actual}/10</div>`;
      }
    }
  ...
  function getTopProperties(material) {
    ...
    const top = properties.filter(property => material[property] >= 8).map(capitalize);
  ...
  function getWeakProperties(material) {
    const properties = ["strength", "lightness", "stiffness", "flexibility", "cost", "sustainability"];
    const weak = properties.filter(property => material[property] <= 3).map(capitalize);
  ...
  function createTradeoffExplanation(material, mission, score) {
    return `
      <div class="tradeoff-box">
        ...
        <p>
          For <strong>${mission.name}</strong>, <strong>${material.name}</strong> scored <strong>${score}/100</strong>.
          Its strongest areas are <strong>${getTopProperties(material).join(", ")}</strong>.
          Its weaker areas are <strong>${getWeakProperties(material).join(", ")}</strong>.
        </p>
      </div>
    `;
  }
  ...
  function createEngineerDetails(material, weights) {
    let html = `
      <div class="engineer-details">
        ...
        <p class="small-note">
          This mode shows how the score was calculated. A material gets more points when it is strong in a category that the mission cares about.
        </p>
    ...
    html += `
        ...
        <p><strong>Engineering data note:</strong> ${material.engineerNote}</p>
      </div>
    `;
  ...
  function testDesign() {
    ...
    const score = calculateScore(material, weights);

    let html = `
      <p class="score">${score}/100</p>
      <h3>${getScoreMessage(score)}</h3>
      <p><strong>Mission:</strong> ${mission.name}</p>
      <p><strong>Material:</strong> ${material.name}</p>
      <p><strong>Why?</strong> ${material.explanation}</p>
      ${createChecklist(material, mission)}
      ${createTradeoffExplanation(material, mission, score)}
    `;

    if (modeSelect.value === "engineer") {
      html += createEngineerDetails(material, weights);
    } else {
      html += `
        <div class="engineer-details">
          <h3>Beginner Mode</h3>
          <p>
            This material was scored using simple engineering ideas like strength, weight, stiffness, flexibility, cost, and sustainability.
            Switch to Engineer Mode to see the scoring table.
          </p>
        </div>
      `;
    }
  ...
  function findBestMaterial() {
    ...
    const bestMaterial = materials[bestKey];
    ...
    recommendBox.innerHTML = `
      <div class="recommendation">
        ...
        <p>
          This recommendation is based on your current mission and slider choices.
        </p>
      </div>
    `;
  }
  ...
  function compareMaterials() {
    ...
    let winner = "Tie";
    if (scoreA > scoreB) winner = materialA.name;
    if (scoreB > scoreA) winner = materialB.name;
    ...
    compareBox.innerHTML = `
      ...
      <div class="tradeoff-box">
        ...
        <p><strong>${materialA.name}:</strong> ${materialA.explanation}</p>
        <p><strong>${materialB.name}:</strong> ${materialB.explanation}</p>
      </div>
    `;
  }

  loadDropdown(materialSelect, "plastic");
  loadDropdown(compareA, "steel");
  loadDropdown(compareB, "aluminum");
